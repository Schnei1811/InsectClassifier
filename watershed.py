...
def watershed(img_dir_list, args):
    ...
    extracted_images_dir = f"extracted\\"

    black_white_img_dir = f"interm_images/Black_White_div_{args.div}\\"
    boxes_img_dir = f"interm_images/Boxes_div_{args.div}\\"
    contours_img_dir = f"interm_images/Contours_div_{args.div}\\"
    grayscale_img_dir = f"interm_images/Greyscale_div_{args.div}\\"
    mask_img_dir = f"interm_images/Mask_div_{args.div}\\"
    density_dir = f"interm_images/Density_div_{args.div}\\"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--image_dir", type=str, default="G:PythonData\\ALUS\\21_03_13_Sheet\\JPG")
    ...
    img_dir_list = glob(f"{args.image_dir}\\*")
    ...
    if (args.multiprocess == True):
        mp_list_size = int(len(img_dir_list) / (mp.cpu_count() - 1))
