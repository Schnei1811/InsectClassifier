    ...
    for epoch in range(epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                ...
                # Save best model
                fname = f"models/{data_name}/{arch}/{fold}_{best_acc}_{epoch}.pt"
...
def test_model(model, dataloaders_dict, device, fold, test_img_paths, data_name, xp_description, model_name, mixed, is_inception=False):
    since = time.time()
...
def createData(X_paths, Y, data_name):
    ...
    if not os.path.exists("metadata/Arrays_Batches"):
        os.makedirs("metadata/Arrays_Batches")

    if not os.path.exists("metadata/Arrays_Data"):
        os.makedirs("metadata/Arrays_Data")
    ...
    for batch in range(data_batch):
        data_paths.append(glob(f'metadata/Arrays_Batches/{data_name}_Input_{batch}_*')[0])
    ...
    np.save(f'metadata/Arrays_Data/{data_name}_Input_{len(X)}.npy', X)
    np.save(f'metadata/Arrays_Data/{data_name}_Labels_{len(Y)}.npy', Y)
...
def createCSV(csv_path, data_name):
    ...
    for file_path in tqdm(glob(f"extracted/{data_name}/*")):
        print(file_path)
        species = "_".join(file_path.split("/")[-1].split("_")[:2])
        print(species)
...
def main(data_path, data_name, arch, batch_size, fold, xp, epochs):
    ...
    with open(f"metadata/{data_name}_num_to_class.json", "w") as fp:
    ...
    input_file_path = f"metadata/Arrays_Data/{data_name}_Input_{len(X_paths)}.npy"
    label_file_path = f"metadata/Arrays_Data/{data_name}_Labels_{len(X_paths)}.npy"
    ...
    #     if not ckpt:
    #         try:
    #             model = sorted(glob(f"Models/{data_name}/{xp_description}/{model_name}/{fold}_*.pt"))[-1]
    #         except IndexError as e:
    #             print(e, data_name, xp_description, model_name, fold)
    #             quit()
    #         model_ft.load_state_dict(torch.load(model))
    #     else:
    #         model_ft.load_state_dict(torch.load(ckpt))
    #     print(f"\nLoading model {model}")
    #     test_img_paths = X_paths[kfolds[fold][TEST]]
    #     test_model(model_ft, dataloaders_dict, device, fold, test_img_paths, data_name, xp_description, model_name, mixed, is_inception=(model_name=="inception"))
    # else:
    #     print("Select xp as either 'train' or 'test'")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--xp", default="train")                #train or test
    parser.add_argument("--epochs", default=25, type=int)
    args = parser.parse_args()

    main(args.data_path, args.data_name, args.arch, args.batch_size, args.fold, args.xp, args.epochs)